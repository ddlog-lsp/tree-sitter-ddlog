...
unsigned tree_sitter_ddlog_dl_external_scanner_serialize(const void *const payload,
                                                   char *const buffer) {
  (void)payload;
  (void)buffer;
  return 0;
...
void tree_sitter_ddlog_dl_external_scanner_deserialize(const void *const payload,
                                                 char *const buffer,
...
static bool tree_sitter_ddlog_dl_is_end_of_token(TSLexer *const lexer) {
  return lexer->lookahead == 0 || iswspace(lexer->lookahead) ||
         lexer->lookahead == '/';
}

static void tree_sitter_ddlog_dl_skip_extras(TSLexer *const lexer) {
...
bool tree_sitter_ddlog_dl_external_scanner_scan(const void *const payload,
                                          TSLexer *const lexer,
